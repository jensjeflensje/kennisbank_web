...
def edit_question(request, guild_id, guild_data, guild_obj, question_id=0):
    question = Question.objects.filter(id=question_id, guild__discord_id=guild_id).first()
    if request.method == "POST":
...
def edit_channel(request, guild_id, guild_data, guild_obj, channel_id=0):
    channel = GuildChannel.objects.filter(id=channel_id, guild__discord_id=guild_id).first()
    if request.method == "POST":
